<div>
        ...
        <div class="mb-1 w-full">
            ...
            <div class="block sm:flex items-center md:divide-x md:divide-gray-100">
                ...
                <div class="mt-1 relative sm:w-64 xl:w-96">
                    <input wire:model.debounce.500ms="q" type="search" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5" placeholder="Search for products">
                </div>
                        ...
                        <thead class="bg-gray-100">
                            <tr>
                                ...
                                <th scope="col" class="p-4 text-left text-xs font-medium text-gray-500 uppercase">
                                    <button wire:click="sortBy('name')">Product Name</button>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach ($products as $product)
                                <tr class="hover:bg-gray-100">
                                    ...
                                    <td class="p-4 whitespace-nowrap text-base font-medium text-gray-900">{{$product->formatPrice() }}</td>
                                    <td class="p-4 whitespace-nowrap space-x-2">
                                        <button wire:click="confirmingProductEdit({{ $product->id }})" wire:loading.attr="disabled"
                                            type="button" class="text-white bg-cyan-600 hover:bg-cyan-700 focus:ring-4 focus:ring-cyan-200 font-medium rounded-lg text-sm inline-flex items-center px-3 py-2 text-center">
                                            <svg class="mr-2 h-5 w-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M17.414 2.586a2 2 0 00-2.828 0L7 10.172V13h2.828l7.586-7.586a2 2 0 000-2.828z"></path><path fill-rule="evenodd" d="M2 6a2 2 0 012-2h4a1 1 0 010 2H4v10h10v-4a1 1 0 112 0v4a2 2 0 01-2 2H4a2 2 0 01-2-2V6z" clip-rule="evenodd"></path></svg>
                                            Edit item
                                        </button>
                                        <button wire:click="confirmProductDeletion({{ $product->id }})" wire:loading.attr="disabled"
                                            type="button" class="text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm inline-flex items-center px-3 py-2 text-center">
        ...
        <x-slot name="title">
            <div class="flex items-start justify-between p-5 border-b rounded-t">
                <h3 class="text-xl font-semibold">
                    {{ isset($this->product->id) ? 'Edit ' : 'Add ' }} Product
                </h3>
                <button wire:click="$toggle('confirmingProductAdd')" type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center" data-modal-toggle="add-product-modal">
            ...
            <div class="p-6 space-y-6">
                <div class="grid grid-cols-6 gap-6">
                    <div class="col-span-6 sm:col-span-3">
                        ...
                        <input type="text" name="name" id="name" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5" wire:model.defer="product.name" required>
                    </div>
                    {{-- <div class="col-span-6 sm:col-span-3">
                    ...
                    <div class="col-span-6 sm:col-span-3">
                        ...
                        <input type="number" name="price" id="price" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5" wire:model.defer="product.price" required>
                    </div>
                    <div class="col-span-full">
                        <label for="description" class="text-sm font-medium text-gray-900 block mb-2">Product Details</label>
                        <textarea name="description" id="description" rows="6" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-4" wire:model.defer="product.description"></textarea>
                    </div>
                </div>
            </div>
        </x-slot>
        <x-slot name="footer">
            <button wire:click="saveProduct()" class="text-white bg-cyan-600 hover:bg-cyan-700 focus:ring-4 focus:ring-cyan-200 font-medium rounded-lg text-sm px-5 py-2.5 text-center" type="submit">
                {{ isset($this->product->id) ? 'Edit ' : 'Add ' }} product
            </button>
        </x-slot>
    </x-dialog-modal>
    ...
    <x-dialog-modal wire:model="confirmingProductDeletion">
        <x-slot name="title">#{{ $product->id }}</x-slot>
        <x-slot name="content">
            ...
            <h3 class="text-xl text-center font-normal text-gray-500 mt-5 mb-6">Are you sure you want to delete this product?</h3>
        </x-slot>
        <x-slot name="footer">
            <button wire:click="deleteProduct({{ $this->confirmingProductDeletion }})" class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-base inline-flex items-center px-3 py-2.5 text-center mr-2">
                Yes, I'm sure
            </button>
            <button wire:click="$set('confirmingProductDeletion', false)" class="text-gray-900 bg-white hover:bg-gray-100 focus:ring-4 focus:ring-cyan-200 border border-gray-200 font-medium inline-flex items-center rounded-lg text-base px-3 py-2.5 text-center" data-modal-toggle="delete-product-modal">
                No, cancel
            </button>
        </x-slot>
    </x-dialog-modal>
