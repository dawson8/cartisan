<div>
        ...
        <div class="mb-1 w-full">
            <div class="mb-4">
                <nav class="flex mb-5" aria-label="Breadcrumb">
                    ...
                    <li>
                        <div class="flex items-center">
                            <svg class="w-6 h-6 text-gray-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg>
                            <span class="text-gray-400 ml-1 md:ml-2 text-sm font-medium" aria-current="page">Categories</span>
                        </div>
                    </li>
                    </ol>
                </nav>
            ...
            <div class="block sm:flex items-center md:divide-x md:divide-gray-100">
                ...
                <div class="flex items-center sm:justify-end w-full">
                    ...
                    <button wire:click="confirmingCategoryAdd"
                        type="button"
                        ...
                        <thead class="bg-gray-100">
                            <tr>
                                ...
                                <th scope="col" class="p-4 text-left text-xs font-medium text-gray-500 uppercase">
                                    <button wire:click="sortBy('name')">Category Name</button>
                            ...
                            @foreach ($categories as $category)
                                <tr class="hover:bg-gray-100">
                                    ...
                                    <td class="p-4 whitespace-nowrap space-x-2">
                                        <button wire:click="confirmingCategoryEdit({{ $category->id }})" wire:loading.attr="disabled"
    ...
    <x-dialog-modal wire:model="confirmingCategoryAdd">
        <x-slot name="title">
            <div class="flex items-start justify-between p-5 border-b rounded-t">
                <h3 class="text-xl font-semibold">
                    {{ isset($this->category->id) ? 'Edit ' : 'Add ' }} Category
                </h3>
                <button wire:click="$toggle('confirmingCategoryAdd')" type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center">
            ...
            <div class="p-6 space-y-6">
                <div class="grid grid-cols-6 gap-6">
                    <div class="col-span-6 sm:col-span-3">
                        ...
                        <input type="text" name="name" id="name" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5" wire:model.defer="category.name" required>
                    </div>
        ...
        <x-slot name="footer">
            <button wire:click="saveCategory()" class="text-white bg-cyan-600 hover:bg-cyan-700 focus:ring-4 focus:ring-cyan-200 font-medium rounded-lg text-sm px-5 py-2.5 text-center" type="submit">
                {{ isset($this->category->id) ? 'Edit ' : 'Add ' }} product
            </button>
        </x-slot>
    </x-dialog-modal>
    ...
    <x-dialog-modal wire:model="confirmingCategoryDeletion">
        <x-slot name="title">#{{ $category->id }}</x-slot>
        <x-slot name="content">
            ...
            <h3 class="text-xl text-center font-normal text-gray-500 mt-5 mb-6">Are you sure you want to delete this category?</h3>
        </x-slot>
        <x-slot name="footer">
            <button wire:click="deleteCategory({{ $this->confirmingCategoryDeletion }})" class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-base inline-flex items-center px-3 py-2.5 text-center mr-2">
                Yes, I'm sure
            </button>
            <button wire:click="$set('confirmingCategoryDeletion', false)" class="text-gray-900 bg-white hover:bg-gray-100 focus:ring-4 focus:ring-cyan-200 border border-gray-200 font-medium inline-flex items-center rounded-lg text-base px-3 py-2.5 text-center">
                No, cancel
            </button>
        </x-slot>
    </x-dialog-modal>
